...
one_map_noproj(fig, 8,2,3,  ssh,   'm', tmask0, 'tab20c', -0.2, 0, 'init sea surface height eORCA36 ');
...
one_map_noproj(fig, 8,2,5,  flx2,   'W/m2', tmask0, cmocean.cm.balance, -200, 200, 'init net heat flux eORCA36 ');
...
one_map_noproj(fig, 8,2,8,  flx5,   'm/s', tmask0, cmocean.cm.speed, 0, 2, 'init x-wind stress eORCA36 ');
flx6=get_data2D(filein,'sometauy',0,-1)
one_map_noproj(fig, 8,2,9,  flx6,   'm/S', tmask0, cmocean.cm.speed, 0, 2, 'init y-wind stress eORCA36 ');

dep=100
lev=np.where(np.abs(navlev-dep)==np.min(np.abs(navlev-dep)))[0]
tmask100=dsmask['tmask'][0,lev]
temp100=get_data2D(filein,'votemper',0,lev)
one_map_noproj(fig, 8,2,10,  temp100,   '°C', tmask100, cmocean.cm.thermal, -5, 30, 'init abort temperature eORCA36 100m');
salt100=get_data2D(filein,'vosaline',0,lev)
one_map_noproj(fig, 8,2,11,  salt100,   'PSI', tmask100, cmocean.cm.haline, 30, 35, 'init abort salinity eORCA36 100m');

dep=500
lev=np.where(np.abs(navlev-dep)==np.min(np.abs(navlev-dep)))[0]
tmask500=dsmask['tmask'][0,lev]
temp500=get_data2D(filein,'votemper',0,lev)
one_map_noproj(fig, 8,2,12,  temp500,   '°C', tmask500, cmocean.cm.thermal, -5, 30, 'init abort temperature eORCA36 500m');
salt500=get_data2D(filein,'vosaline',0,lev)
one_map_noproj(fig, 8,2,13,  salt500,   'PSI', tmask500, cmocean.cm.haline, 30, 35, 'init abort salinity eORCA36 500m');

dep=1000
lev=np.where(np.abs(navlev-dep)==np.min(np.abs(navlev-dep)))[0]
tmask1000=dsmask['tmask'][0,lev]
temp1000=get_data2D(filein,'votemper',0,lev)
one_map_noproj(fig, 8,2,14,  temp1000,   '°C', tmask1000, cmocean.cm.thermal, -5, 30, 'init abort temperature eORCA36 1000m');
salt1000=get_data2D(filein,'vosaline',0,lev)
one_map_noproj(fig, 8,2,15,  salt1000,   'PSI', tmask1000, cmocean.cm.haline, 30, 35, 'init abort salinity eORCA36 1000m');
